...
from utils.plant_validator import is_plant_image
...
def _validate_and_save(file_data, ext):
    """Save uploaded image to disk, then validate it's a plant image."""
    ...
    # Run plant validation
    validation = is_plant_image(filepath)

    if not validation["is_plant"]:
        # Delete the rejected image — no point keeping it
        try:
            os.remove(filepath)
        except OSError:
            pass
    ...
    return filename, filepath, validation
...
def predict_upload():
    ...
    Validates it's a plant image, then runs CNN disease inference.
    """
    ...
    # Save & validate
    filename, filepath, validation = _validate_and_save(file, ext)

    if not validation["is_plant"]:
        return jsonify({
            "error":       "not_a_plant",
            "message":     validation["message"],
            "detected_as": validation["top_label"],
        }), 422
    ...
    return jsonify({
        ...
        "prediction": {
            **result,
            ...
            "plant_confidence":    validation["confidence"],
        },
...
def predict_camera():
    ...
    # Save & validate
    filename, filepath, validation = _validate_and_save(image_bytes, "jpg")
