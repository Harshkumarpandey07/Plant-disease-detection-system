        ...
        with open(class_names_path) as f:
            ...
            for name in raw:
                ...
                if len(parts) == 2:
                    ...
                    _class_names.append(f"{crop} — {'Healthy' if disease.lower()=='healthy' else disease}")
                else:
...
def _preprocess(image_path, img_size):
    ...
    # NO normalization — model has Rescaling(1/255) built in as first layer
...
def _mock_predict(image_path):
    ...
    for i, ci in enumerate(indices):
        if i == 0: conf = confidence
        elif i == 1: conf = round(remaining * rng.uniform(0.4, 0.7), 4)
        else: conf = round(remaining - top3[1]["confidence"], 4)
        remaining -= conf
